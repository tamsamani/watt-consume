...
export default function({ parent, ...props }) {
	...
	const [hookId, hook] = useState(0);
	const useHook = () => hook(hookId + 1);
	let iconId = parent.store.selectedIcon;
	...
	>
		<Button raised
			text={<div>Icon {hookId} : <Icon>{Icons[iconId]}</Icon></div>}di
			onClick={e=>parent.actions.showIconSet(true, useHook)}
		/>
